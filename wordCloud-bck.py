...
dataframeBR = pd.DataFrame(list(db.recipesIngredients.find({"id": "1"})))
...
for i in range(0,7794):
     if(dataframeBR.loc[i, 'score'] > 35.0):
         if(dataframeBR.loc[i, 'score'] > 0.0):
...
wordcloud = WordCloud(collocations=False, width=480, height=480, margin=0, background_color = 'black').generate(text.replace("\'", "").replace("picada", "").replace("cortada", "").replace("pedaços", ""))
...
dataframeIN = pd.DataFrame(list(db.recipesIngredients.find({"id": "5"})))

for i in range(0,966):
    if (dataframeIN.loc[i, 'score'] < 15.0 and dataframeIN.loc[i, 'score'] > 0.0):
        ...
        stopWords.update(
               ["cup", "teaspoon", "teaspoons", "tablespoon", "tablespoons", "ounce", "chopped", "sliced", "cups",
                ...
                "paste", "needed", "green", "tsp", "water", "grated", "xc2xbd", "powder", "tbsp", "e", "a", "i", "'", " "], "1/2", "1/4", "black", "colheres","pitada", "suco","molho", "picado","2'", "2 '" "picada'","pó", "colher", "xícara", "xícaras", "\'", "sopa", "lata", "caixinha",
                "caixa", "chá", "dentes", "gosto", ",", "1", "2", "2 '", "3", "4", "5", "6", "creme", "reino", "branco", "ml", "kg", "g", "dente")
#         )
        words = word_tokenize(data)

        for w in words:
           if w not in stopWords:
               if len(w) > 2 and w != "1/2" and w != "1/4" and w != "paste" and w != "large" and w != "'275g" and w != "black" and w != "crushed" and w != "mashed" and w != "xícara" \
                       and w != "colheres" and w != "sopa" and w != "xícaras" and w != "chá" and w != "colher" and w != "sem"\
                       and w != "lata" and w != "creme" and w != "para" and w != "condensado" and w != "suco":
...
dataframeUSA = pd.DataFrame(list(db.recipesIngredients.find({"id": "6"})))

for i in range(0,12167):
    if (dataframeUSA.loc[i, 'score'] > 35.0 and dataframeUSA.loc[i, 'score'] > 0.0):
        data = (str(dataframeUSA.loc[i, "ingredients"]))
        stopWords = set(stopwords.words('english'))
        stopWords.update(
               ["cup", "teaspoon", "teaspoons", "tablespoon", "tablespoons", "ounce", "chopped", "sliced", "cups",
                 "taste", "diced", "white", "pound", "ground", "caster", "100g", "fresh", "finely", "175g", "leaves", "red", "xc2xbd"
