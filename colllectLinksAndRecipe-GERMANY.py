...
# TypesOfRecipes = {
#     "hauptgericht": 612,     #prato principal
#     "dessert-und-suesses": 193,
#     "fruehstueck": 77,      #café da manhã
#     "salat": 186,
#     "vorspeise": 127        #aperitivos
# }
...
while i < len(listTypes):
    ...
    for item in range(1, TypesOfRecipes[category]):
        url = "http://de.allrecipes.com/rezepte/" + category + "-rezepte.aspx?page=" + str(item)
    ...
    with open(listTypes[i] +".txt") as file:
        ...
        for line in file:
            ...
            while w < len(peopleWhoMade):
                numberPeopleWhoMade = peopleWhoMade[w].find("span").text.strip("anderen gemacht").strip("Von")
                    ...
                    while q < len(timing):
                        ...
                        if len(parameters) != 0:
                            while v < len(parameters):
                                ...
                                if "Zubereitung" in str(parameters[v]):
                                    prepTime = parameters[q].find(class_="accent").text
                                if "Kochzeit" in str(parameters[v]):
