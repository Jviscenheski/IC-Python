...
# meu método para criar os arrays
def criaArrays(arquivo):
    ...
    with open(arquivo) as file:
        for line in file:
            ...
            listIndia.append(line.split(":")[4])
            listEUA.append(line.split(":")[5].replace("\n", ""))
...
if __name__ == "__main__":
    ...
    for g in np.unique(group):
        ix = np.where(group == g)
        pylab.scatter(Y[ix, 0], Y[ix, 1], 20, c=cdict[g], label=g)
