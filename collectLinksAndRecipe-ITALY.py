...
#         url = "http://allrecipes.it/ricette/" + category + "-ricette.aspx?page=" + str(item)
#
#         # novo
...
for i, item in enumerate(listTypes):
    ...
    with open(listTypes[i] +".txt") as file:
        ...
        for line in file:
            ...
            name = soupRecipe.find("span", {"itemprop": "name"}).text.strip().replace("è", "e")
            ...
            while w < len(peopleWhoMade):
                numberPeopleWhoMade = peopleWhoMade[w].find("span").text.strip("persone hanno provato questa ricetta")
                    ...
                    while q < len(timing):
                        ...
                        if len(parameters) != 0:
                            while v < len(parameters):
                                ...
                                if "Preparazione" in str(parameters[v]):
                                    prepTime = parameters[q].find(class_="accent").text
                                if "Cottura" in str(parameters[v]):
