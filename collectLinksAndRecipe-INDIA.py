...
while i < len(listTypes):
    ...
    for item in range(1, TypesOfRecipes[category]):
        url = "http://allrecipes.co.in/recipes/" + category + "-recipes.aspx?page=" + str(item)

        if listTypes[i] == "salad":
            url = "http://allrecipes.co.in/recipes/" + linkSaladas + category + "-recipes.aspx?page=" + str(item)
    ...
    with open(listTypes[i] +".txt") as file:
        ...
        for line in file:
            ...
            while w < len(peopleWhoMade):
                numberPeopleWhoMade = peopleWhoMade[w].find("span").text.strip("people made this")
                    ...
                    while q < len(timing):
                        ...
                        if len(parameters) != 0:
                            while v < len(parameters):
                                ...
                                if "Prep" in str(parameters[v]):
                                    prepTime = parameters[q].find(class_="accent").text
                                if "Cook" in str(parameters[v]):
